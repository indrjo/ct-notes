...
\NotaInterna{Expand this section with details about duality and \TeX{} some initial exercises.}
...
\[k_v : \cat I \to \cat C\]
  where \(k_v(i) := v\) for every object \(i\) and \(k_v(f) := \id_v\) for every morphism \(f\). \NotaInterna{Later, in the chapter of the adjunctions, we will introduce one functor \(\Delta : \cat C \to [\cat I, \cat C]\).} A {\em limit} of a functor \(F : \cat I \to \cat C\) is some object \(v\) of \(\cat C\) with a natural transformation \(\lambda : k_v \tto F\)
such that:
%\begin{quotation}
...
%In general, (co)cones can be twisted things and (co)limits, if exist, can be even more so. Yes, for a functor \(\cat I \to \cat C\), the category \(\cat C\) has its share, but it is \(\cat I\) who has the last say in the research of (co)limits. The role of \(\cat I\) is to give a \q{shape} of the limits we are looking for, indeed.
...
%\newline
%Let us now say what limits and colimits of \(H : \naturals \to \cat C\) are.
\end{example}
...
\end{quotation}
\NotaInterna{Is that thing even interesting?} \NotaInterna{Write about functors \(\cat G \to \Set\)\dots{}}
\end{example}

\noindent\NotaInterna{Write about duality here. Explain how limits and colimits are dual\dots{}}
