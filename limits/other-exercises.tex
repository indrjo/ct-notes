...
% Here is a collection of exercises that do have a precise placement
% in the the previous sections, for they deal with the interplay
% between different kinds of limits and with the previous chapters in
% a manner that would burden the flow of the discourse.
...
\begin{exercise}[Injective \& surjective morphisms]
...
%
  \item \NotaInterna{The converse, epic \(\tto\) surjective, under
      additional assumptions about \(\cat C\) requires some work which
      is not trivial. Decompose in smaller part and let the reader
      draw the conclusion.}
  \end{tcbenum}
\end{exercise}
...
\begin{exercise}[Natural number objects]
  ...
  \[\begin{tikzcd}
      1 \ar["0", r] & \naturals \ar["s", r] & \naturals
    \end{tikzcd}\] \NotaInterna{I should spend more words for natural
    number objects.}  It is not necessarily the \(\naturals\) in
  ...
  \begin{quotation}
    ...
    \[\begin{tikzcd}[row sep=small]
      ...
      \end{tikzcd}\] \NotaInterna{We just used some notation we have
      never used before here.}
  \end{quotation}
  ...
  {\em recursion theorem} is equivalent to {\em primitive recursion
    theorem}. \NotaInterna{Prepare a hint.}
\end{exercise}
