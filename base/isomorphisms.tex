...
%   Cantor, the father of Set Theory, conducted its enquiry on
%   cardinalities and not on equality of sets. For \(A\) and \(B\)
...
%   \end{tcbenum}
%   Though they are logically equivalent, they differ in some
%   sense. In Set Theory, the adjective \q{bijective} is defined by
%   referring of the fact that sets are things that have elements:
%   \begin{quotation}
%     for every \(y \in B\) there is one and only one \(x \in A\) such
%     that \(f(x) = y\).
%   \end{quotation}
%   In contrast, (2) is a statement written in terms of functions and
%   compositions of functions: so (2) is written in a categorial
%   language.
% \end{example}
%
%\begin{exercise}
...
\begin{example}[Isomorphisms of sets]
  ...
  Set Theory, the adjective \q{bijective} is defined by referring of
...
\begin{example}[Isomorphisms in \(\Top\)]
  ...
  compact while \([0,1)\) is not. \q{Fortunately}, in Topology there
  are two basic facts:
  \begin{itemize}
...
\NotaInterna{Heavy work in progress here\dots{}} In categories one is
...
\begin{figure}
  ...
  \begin{tikzcd}
    a \ar["f", rr] & & b \\
    & c \ar["g", ""{name=g, above}, ul] \ar["fg", ""{name=fg, above},
    ur, swap] \ar[bend left, from=g, to=fg]
  \end{tikzcd}
  \begin{tikzcd}
...
Consider a \NotaInterna{locally small? if our categories are all
  locally small by default, then no problem here\dots{}} category
...
Frankly, the notations \q{\(f \circ -\)} and \q{\(- \circ f\)} may be
...
\NotaInterna{Wait\dots{} \q{type signature}?} Here,
...
However, that symbolism is quite advantageous since it makes easier to
...
and their commutativity. \NotaInterna{\TeX{} the part that comes
  here!}

Being the relation \(\cong\) an equivalence relation
(\inlinethm{exercise}), the following construction is
...
\NotaInterna{Rewrite.} We are yet at an early stage to formulate this,
